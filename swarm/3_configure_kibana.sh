...
docker service ls --filter "name=dcurl" | grep "dcurl"
if [ $? -ne 0 ]; then
  docker service create --name dcurl --network voteapp --constraint "node.role == manager" alexeiled/alpine-plus:3.4 tail -f /dev/null
fi

dcurl_status=""
while [ "$dcurl_status" != "Running" ]; do
  dcurl_status=$(docker service ps -f "name=dcurl" dcurl | grep -w 'Running' | awk '{print $6}')
  sleep 2
done
CURL_CONTAINER=$(docker ps --filter "label=com.docker.swarm.service.name=dcurl" -q)
dcurl="docker exec -it ${CURL_CONTAINER} curl"

# Wait until ES is ready
$dcurl -XGET http://${ES_HOST}:9200/_cluster/health?pretty=true | grep "green\|yellow"
while [ $? -ne 0 ]; do
  sleep 2
...
$dcurl -H "Content-Type: application/json" -XPOST -d '{
...
$dcurl -XGET http://${ES_HOST}:9200/_cat/indices | grep kibana | grep "yellow\|green"
while [ $? -ne 0 ]; do
  sleep 2
...
$dcurl -XPUT http://${ES_HOST}:9200/.kibana/_mapping/search -d '
  {
    "properties":{
      "columns":{"type":"string"},
...
$dcurl -XPUT http://${ES_HOST}:9200/.kibana/_mapping/index-pattern -d '
...
$dcurl -XPUT http://${ES_HOST}:9200/.kibana/_mapping/dashboard -d '
...
$dcurl -XPUT http://${ES_HOST}:9200/.kibana/_mapping/visualization -d '
