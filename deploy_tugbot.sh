...
if [ $? -ne 0 ]; then
  docker service create --name tugbot-collect --network voteapp --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock gaiadocker/tugbot-collect:latest tugbot-collect -g http://tugbot-result-service:8080/results -c null
fi
